...
- Convert Markdown to HTML with optional theme styling
- Convert Markdown to a Word (`.docx`) document
- Both converters accept either an uploaded `.md` file or Markdown text pasted straight into the page
...
- `utils/` – Helper functions and utilities used across blueprints:
  ...
  - `validators.py` – Shared input validation, including `resolve_markdown_input()` used by both Markdown converters
