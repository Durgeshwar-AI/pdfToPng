  ...
  return (
    <nav className="fixed top-0 left-0 z-9999 w-full bg-white/50 backdrop-blur shadow-sm">
      <div className="max-w-7xl mx-auto px-6 h-19 flex justify-between items-center">
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`relative font-semibold hover:text-purple-600 py-2 px-4 rounded-xl text-lg transition-all duration-300 hover:bg-purple-100 hover:scale-105 ${
                isActive(item.link) ? "text-purple-600" : "text-slate-700"
              }`}
            >
              {item.name}
            </Link>
        ...
        <div
          className={`fixed inset-x-4 top-20 z-50 lg:hidden rounded-2xl border border-gray-100 bg-white/95 backdrop-blur-xl shadow-2xl p-4 flex flex-col gap-2 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] transform ${isMenuOpen ? "opacity-100 translate-y-0 scale-100" : "opacity-0 -translate-y-6 scale-95 pointer-events-none"}`}
        >
          {navItems.map((item) => (
            <Link
              key={item.name}
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-slate-900 shadow-sm border border-slate-900 text-white hover:shadow-md transition-all duration-300 hover:scale-105"
          >
