...
export default function PdfSplit() {
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      setResultUrl(url);
      toast.success(
        `${pageIndices.length} page${pageIndices.length !== 1 ? 's' : ''} extracted successfully`
      );
    } catch (err) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl rounded-3xl border border-slate-200 bg-white p-8 shadow-xl">
      <h1 className="mb-3 text-center text-5xl font-bold">Split PDF</h1>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div>
          <div
            ...
            className={cn(
              'cursor-pointer rounded-3xl border-2 border-dashed p-10 transition-all',
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {totalPages} pages
                    {deletedPages.size > 0 && (
                      <span className="ml-2 text-red-400">· {deletedPages.size} removed</span>
                    )}
                  </p>
                </div>
              ...
              <div className="mb-4 flex items-center gap-2">
                ...
                {deletedPages.size > 0 && (
                  <span className="ml-auto text-xs font-medium text-red-400">
                    {deletedPages.size} page{deletedPages.size !== 1 ? 's' : ''} removed — hover to
                    restore
                  </span>
                )}
              </div>

              <div className="grid max-h-[450px] grid-cols-2 gap-4 overflow-y-auto sm:grid-cols-3">
                {previews.map(page => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        'group relative cursor-pointer rounded-xl border p-2 transition-all duration-200',
                        isDeleted
                    ...
                    >
                      ...
                      {inRange && !isDeleted && (
                        <div className="absolute top-3 left-3 rounded-full bg-blue-500 px-2 py-0.5 text-[10px] font-bold text-white">
                          Selected
                        </div>
                      )}
                      ...
                      <p className="mt-2 text-center text-xs">Page {page.pageNum}</p>

                      {!isDeleted && (
                        <button
                          onClick={e => deletePage(e, page.pageNum)}
                          className="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-full bg-white text-gray-500 opacity-0 shadow-md transition-opacity group-hover:opacity-100 hover:bg-red-500 hover:text-white"
                      ...
                      {isDeleted && (
                        <button
                          ...
                          className="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-full bg-green-500 text-white opacity-0 shadow-md transition-opacity group-hover:opacity-100 hover:bg-green-600"
        ...
        <div>
          <div className="rounded-3xl border p-8">
            ...
            {error && (
              <div className="mt-6 flex items-center gap-2 rounded-xl bg-red-50 p-4 text-red-500">
                <AlertCircle size={16} />
                {error}
              </div>
            )}
            ...
            <button
              ...
              className="mt-6 w-full rounded-2xl bg-blue-600 py-4 font-semibold text-white disabled:opacity-50"
            >
      ...
      {previewModal && (
        ...
        >
          <div
            className="relative max-h-[90vh] max-w-3xl overflow-auto rounded-2xl bg-white p-4 shadow-2xl"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={() => setPreviewModal(null)}
              className="absolute top-3 right-3 flex h-9 w-9 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600"
