...
function PdfToPptx() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const loadingId = toastLoading(`Converting "${file.name}" to PPTX…`);

    const formData = new FormData();
    formData.append("file", file);

    try {
      const response = await fetch(`${BACKEND_URL}/convertPdfToPptx`, {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        toastSuccess("Your PPTX has been downloaded!");
      } else {
  ...
  return (
    <div className="w-full max-w-[700px] mx-auto p-10 text-center flex flex-col justify-center items-center theme-panel rounded-2xl overflow-hidden">
      ...
      <p className="text-gray-500 text-sm mb-8 -mt-6">Upload a .pdf file to convert it to an editable PowerPoint presentation.</p>
      ...
      <form onSubmit={handleSubmit} className="w-full flex flex-col items-center">
        <FileUploadArea
          ...
          supportText="Each PDF page becomes a slide in the PPTX"
        />
