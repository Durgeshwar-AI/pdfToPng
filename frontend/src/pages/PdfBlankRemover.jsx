...
export default function PdfBlankRemover() {
  ...
  const detectBlankPages = async (pdf) => {
  const blanks = [];
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const ratio = nonWhitePixels / (canvas.width * canvas.height);
  ...
  const pickFile = async (f) => {
    ...
    try {
      ...
      const blanks = await detectBlankPages(pdf);
      setBlankPages(blanks);
      setDeletedPages(new Set(blanks));
      if (blanks.length > 0) {
      toast.success(
    `${blanks.length} blank page${blanks.length !== 1 ? "s" : ""} detected and selected for removal`
  );
} else {
  toast.success("No blank pages detected");
}
  ...
  const handleRemoveBlankPages = async () => {
    ...
    try {
      ...
      setResultUrl(url);
      toast.success(
  `${deletedPages.size} blank page${deletedPages.size !== 1 ? "s" : ""} removed successfully`
);
    } catch (err) {
      ...
      <div className="grid lg:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    ...
                    {deletedPages.size > 0 && (
                      ...
                      · {blankPages.length} blank detected
                      </span>
                    )}
                  </p>
                </div>
            ...
            <div className="mt-6 border rounded-3xl p-4">
              <div className="flex items-center gap-2 mb-4">
                ...
                {deletedPages.size > 0 && (
                  <span className="ml-auto text-xs text-orange-400 font-medium">
                    {deletedPages.size} blank page{deletedPages.size !== 1 ? "s" : ""} detected — hover to restore
                    </span>
                  )}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[450px] overflow-y-auto">
                {previews.map((page) => {
                  ...
                  const isAutoDetected = blankPages.includes(page.pageNum);
        ...
        <div>
          <div className="border rounded-3xl p-8">
            ...
            >
              {isLoading ? "Processing..." : "Remove Blank Pages"}
            </button>
