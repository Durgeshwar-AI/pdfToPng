...
export default function ImageMetadata() {
      ...
      <div className="animate-in fade-in slide-in-from-top-4 mt-8 w-full text-left duration-500">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            ...
            disabled={loading}
            className="flex cursor-pointer items-center gap-2 rounded-lg bg-gradient-to-r from-red-500 to-rose-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:from-red-600 hover:to-rose-700 disabled:cursor-not-allowed disabled:from-gray-300 disabled:to-gray-400"
          >
            <Download size={16} />
            Strip Metadata & Download
          </button>
          <button
            onClick={() => handleCleanAndDownload(file, setLoading)}
            disabled={loading}
            className="flex cursor-pointer items-center gap-2 rounded-lg bg-gradient-to-r from-green-500 to-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:from-green-600 hover:to-emerald-700 disabled:opacity-50"
          >
            <Download size={16} />
            One-Click Clean
          </button>
        ...
        {securityReport && (
          <div className="mb-6 rounded-xl border border-yellow-200 bg-yellow-50 p-4">
            ...
            <div className="mt-3 text-sm text-yellow-900">
              <p className="mb-2 font-medium">Sensitive Data Detected:</p>
        ...
        {keys.length === 0 ? (
          <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 text-center text-sm text-gray-800">
              ...
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    <th className="w-1/3 px-4 py-3 text-xs font-bold tracking-wider text-gray-800 uppercase">
                      Tag / Field
                    </th>
                    <th className="px-4 py-3 text-xs font-bold tracking-wider text-gray-800 uppercase">
                      Value
                    </th>
                    <th className="w-16 px-4 py-3 text-center text-xs font-bold tracking-wider text-gray-800 uppercase">
                      Action
                    </th>
                  </tr>
                </thead>
                  ...
                  {keys.map(key => {
                    ...
                    return (
                      <tr key={key} className="hover:bg-gray-50/50">
                        <td className="px-4 py-3 font-mono text-xs font-semibold break-all text-gray-600">
                          {key}
                        </td>
                        <td className="max-w-xs px-4 py-3 font-mono text-xs break-all text-gray-600">
                          {value}
                        </td>
                        <td className="px-4 py-3 text-center">
                          <button
                            onClick={() => copyToClipboard(key, value)}
                            className="cursor-pointer rounded-lg p-1.5 text-gray-700 transition-colors hover:bg-gray-100 hover:text-gray-600"
  ...
  return (
    <ToolPageTemplate
      ...
      defaultIcon={<Info className="h-16 w-16" />}
