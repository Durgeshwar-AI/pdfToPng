    ...
    <div className="mt-8 w-full text-left">
      <div className="mb-6 rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-500">
          Upload a PNG or JPG, then extract editable text on the client. This is a privacy-focused
          workflow with no server-side OCR.
        </p>
      </div>
      ...
      <div className="mb-4 text-sm text-gray-600">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          {loading && <span className="text-blue-600">{ocrStatus || 'Processing...'}</span>}
        </div>
      </div>
      ...
      <div className="mb-4 rounded-3xl border border-gray-200 bg-white p-4 shadow-sm">
        <textarea
          ...
          disabled={loading}
          className="min-h-[240px] w-full resize-none rounded-3xl border border-gray-200 bg-white p-4 text-sm text-gray-700 transition outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 disabled:bg-gray-50 disabled:text-gray-400"
        />
      </div>
      ...
      <div className="grid gap-3 sm:auto-cols-fr sm:grid-flow-col">
        <button
          ...
          className="inline-flex items-center justify-center rounded-2xl bg-linear-to-r from-[#4361ee] to-[#3b82f6] px-5 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:shadow-[0_10px_25px_rgba(67,97,238,0.2)] disabled:cursor-not-allowed disabled:bg-slate-300 disabled:bg-none disabled:text-slate-700"
        >
          Copy Text
        </button>
        <button
